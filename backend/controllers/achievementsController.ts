...
export const getMyAchievements = async (req: AuthRequest, res: Response) => {
  try {
    if (!req.user) return res.status(401).json({ error: "Must be signed in to view achievements" });
...
export const postAchievements = async (req: AuthRequest, res: Response) => {
  try {
    if (!req.user) return res.status(401).json({ error: "Must be signed in to add achievements" });
    ...
    res.status(200).json({ message: "Achievement added", achievement: result.rows[0] });
  } catch (err) {
    console.error(err);
...
export const putAchievements = async (req: AuthRequest, res: Response) => {
  try {
    if (!req.user) return res.status(401).json({ error: "Must be signed in to update achievements" });
    ...
    if (result.rows.length === 0) return res.status(404).json({ error: "Cannot edit this achievement" });
...
export const deleteAchievements = async (req: AuthRequest, res: Response) => {
  try {
    if (!req.user) return res.status(401).json({ error: "Must be signed in to delete achievements" });
    ...
    if (result.rows.length === 0) return res.status(404).json({ error: "Cannot delete this achievement" });
